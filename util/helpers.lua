...
-- Unique widget-name suffix; math.random can collide across many widgets
local widgetCounter = 0
local function nextWidgetId()
    widgetCounter = widgetCounter + 1
    return "ttp" .. widgetCounter
end
local labelFontSize = 14
local lableFontColor = FONT_COLOR.TITLE -- gold; readable on the dark window shell
...
function helpers.CreateCheckboxWithLabel(parent, anchorInfo, labelText, checkboxText, defaultChecked, onCheckedChanged, tooltipText)
    -- Create label first
    local label = parent:CreateChildWidget("label", "label_" .. nextWidgetId(), 0, true)
    ...
    local checkBox = checkButton.CreateCheckButton("checkbox_" .. nextWidgetId(), parent, checkboxText or "")
    ...
    if tooltipText and tooltipText ~= "" then
        tooltip = helpers.createTooltip("tooltip_" .. nextWidgetId(), checkBox, tooltipText, 0, 0, parent)
    end
...
function helpers.CreateDropdownWithLabel(parent, anchorInfo, labelText, width, options, defaultIndex, onSelectionChanged, tooltipText, tooltipYOffset)
    ...
    local tooltip = nil
    if tooltipText and tooltipText ~= "" then
        tooltip = helpers.createTooltip("tooltip_" .. nextWidgetId(), dropdown, tooltipText, 0, nil, parent)
...
function helpers.CreateTextEditWithLabel(parent, anchorInfo, labelText, width, height, defaultText, isDigitOnly, minValue, maxValue, onTextChanged, tooltipText)
    ...
    local textEdit = W_CTRL.CreateEdit("textEdit_" .. nextWidgetId(), parent)
    ...
    local tooltip = nil
    if tooltipText and tooltipText ~= "" then
        tooltip = helpers.createTooltip("tooltip_" .. nextWidgetId(), textEdit, tooltipText, 0, 0, parent)
    end
...
-- `owner` is the widget the tooltip is created under; `parent` is the control it
-- anchors to and whose hover drives it. Raise() only reorders a widget among its
-- own siblings, so a tooltip owned by a button can never draw above widgets that
-- sit higher in the tree - pass the containing window as `owner` to fix that.
function helpers.createTooltip(id, parent, text, x, y, owner)
    local tooltip = api.Interface:CreateWidget("gametooltip", id, owner or parent)
